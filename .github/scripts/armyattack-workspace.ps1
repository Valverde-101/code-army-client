...
function Merge-ArmyInputCache {
  ...
  if($srcItem.Attributes -band [IO.FileAttributes]::ReparsePoint){
    Remove-ArmyLinkOrTree -Path $Source
    return 0
  }
...
function Remove-ArmyScratchResidue {
  ...
  foreach($child in @(Get-ChildItem -LiteralPath $ScratchRoot -Force -ErrorAction SilentlyContinue)){
    if($child.Name -match '(?i)army|code-army-client'){
...
function Initialize-ArmyAttackWorkspace {
  ...
  # Import/remove legacy Army-owned state. Never touch other projects' roots.
  ...
  foreach($legacyRoot in $legacyRoots){
    ...
    if([IO.Path]::GetFullPath($legacyBuild).TrimEnd('\') -ine [IO.Path]::GetFullPath((Join-Path $runtime 'Builds\code-army-client')).TrimEnd('\')){
      $movedBuilds += (Merge-ArmyBuildGenerations -Source $legacyBuild -Destination $build)
    }
    $legacyInputs=Join-Path $legacyRoot 'Inputs\code-army-client'
    if([IO.Path]::GetFullPath($legacyInputs).TrimEnd('\') -ine [IO.Path]::GetFullPath((Join-Path $runtime 'Inputs\code-army-client')).TrimEnd('\')){
      $movedInputs += (Merge-ArmyInputCache -Source $legacyInputs -Destination $inputCache)
    }
    $scratchRemoved += (Remove-ArmyScratchResidue -ScratchRoot (Join-Path $legacyRoot 'Scratch'))
  }

  # Runtime compatibility facade: project state is local; tool bytes stay global.
  ...
  Ensure-ArmyJunction -Path (Join-Path $runtime 'AndroidSDK') -Target $AndroidSdk
  ...
  # Local AIR50 packaging view using junctioned global SDK components + local manifest.
  ...
  foreach($name in @('platforms','build-tools','platform-tools','licenses')){
    $source=Join-Path $AndroidSdk $name
    if(Test-Path -LiteralPath $source){Ensure-ArmyJunction -Path (Join-Path $packageSdk $name) -Target $source}
  }
  $platformJar=Join-Path $AndroidSdk 'platforms\android-33\android.jar'
  $aapt2=Join-Path $AndroidSdk 'build-tools\33.0.2\aapt2.exe'
  ...
  Write-Host "ARMY_WORKSPACE_LAYOUT=PASS work=$work build=$build input_cache=$inputCache scratch=$scratch runtime=$runtime tools=global-junctions"
