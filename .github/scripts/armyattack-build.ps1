...
# HARMAN AIR 50 legacy packaging expects this alias; point it at the Core pinned SDK instead of copying SDK bytes.
...
# Preserve the stronger Army-specific audits before packaging.
...
# Run the proven Army application builder against the Core-pinned toolchain root.
$legacyBuilder=Join-Path $repoRoot 'Tools\CI\Build-Android.ps1'
Write-Host "ARMY_APPLICATION_BUILD=START adapter=legacy-army-implementation infrastructure=core-3.0.9 render=gpu"
& $legacyBuilder -RepoRoot $repoRoot -ExpectedSha $expected -AndroidBuildRoot ([string]$tc.pinned_android_build_root) -RenderMode gpu
