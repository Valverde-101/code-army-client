...
$impl=Join-Path $PSScriptRoot 'Invoke-AndroidEvidenceRootFixOverlayV48.ps1'
...
if(@($errors).Count -gt 0){
  $errors|ForEach-Object{Write-Host "EVIDENCE_ROOTFIX_V48_PARSER_ERROR line=$($_.Extent.StartLineNumber) message=$($_.Message)"}
  throw 'ANDROID_EVIDENCE_ROOTFIX_V14_COMPAT=FAIL v48_parser_invalid'
}
Write-Host 'ANDROID_EVIDENCE_ROOTFIX_V14_DELEGATE=PASS implementation=v46 adapter=v47+v48 parser=true'
...
try {
  ...
  Write-Host "ANDROID_EVIDENCE_ROOTFIX_V14_COMPAT=PASS delegated=v46 adapters=v47,v48 mode=$Mode sha=$ExpectedSha v4_boundary_fixed=true"
}
finally {
