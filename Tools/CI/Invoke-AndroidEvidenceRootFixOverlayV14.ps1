...
$impl=Join-Path $PSScriptRoot 'Invoke-AndroidEvidenceRootFixOverlayV39.ps1'
...
if(@($errors).Count -gt 0){
  $errors|ForEach-Object{Write-Host "EVIDENCE_ROOTFIX_V39_PARSER_ERROR line=$($_.Extent.StartLineNumber) message=$($_.Message)"}
  throw 'ANDROID_EVIDENCE_ROOTFIX_V14_COMPAT=FAIL v39_parser_invalid'
}
Write-Host 'ANDROID_EVIDENCE_ROOTFIX_V14_DELEGATE=PASS implementation=v39 parser=true'
...
try {
  ...
  Write-Host "ANDROID_EVIDENCE_ROOTFIX_V14_COMPAT=PASS delegated=v39 mode=$Mode sha=$ExpectedSha v4_boundary_fixed=true"
}
finally {
