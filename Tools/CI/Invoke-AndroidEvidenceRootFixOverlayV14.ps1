...
$impl=Join-Path $PSScriptRoot 'Invoke-AndroidEvidenceRootFixOverlayV26.ps1'
...
if(@($errors).Count -gt 0){
  $errors|ForEach-Object{Write-Host "EVIDENCE_ROOTFIX_V26_PARSER_ERROR line=$($_.Extent.StartLineNumber) message=$($_.Message)"}
  throw 'ANDROID_EVIDENCE_ROOTFIX_V14_COMPAT=FAIL v26_parser_invalid'
}
Write-Host 'ANDROID_EVIDENCE_ROOTFIX_V14_DELEGATE=PASS implementation=v26 parser=true'
...
try {
  ...
  Write-Host "ANDROID_EVIDENCE_ROOTFIX_V14_COMPAT=PASS delegated=v26 mode=$Mode sha=$ExpectedSha v4_boundary_fixed=true"
}
finally {
