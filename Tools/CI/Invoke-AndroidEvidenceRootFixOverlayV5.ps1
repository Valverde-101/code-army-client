...
'@ 'installation_campaign_wallclock_arm'
    $enemy=Replace-RegexOne $enemy '(?m)^[ \t]*if\(!wreckingLabelEnd && Config\.OFFLINE_MODE\)[ \t]*Utils\.DiagEvent\("INSTALLATION_WRECKING_CLEANUP","item=" \+ \(mItem \? mItem\.mId : ""\) \+ ";elapsed_ms=" \+ this\.mCampaignWreckingElapsed \+ ";reason=missing_end_label"\);[ \t]*$' @'
