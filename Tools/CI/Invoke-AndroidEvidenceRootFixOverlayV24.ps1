...
if($LASTEXITCODE -ne 0 -or $actual -ne $ExpectedSha){throw "ANDROID_EVIDENCE_ROOTFIX_V24=FAIL exact_head expected=$ExpectedSha actual=$actual"}

$v22=Join-Path $PSScriptRoot 'Invoke-AndroidEvidenceRootFixOverlayV22.ps1'
if(-not(Test-Path -LiteralPath $v22 -PathType Leaf)){throw "ANDROID_EVIDENCE_ROOTFIX_V24=FAIL predecessor_missing=$v22"}
...
$backupRoot=Join-Path $RepoRoot ('.work\scratch\android-evidence-rootfix-v24\'+$ExpectedSha)
$manifestPath=Join-Path $backupRoot 'manifest.json'

function Get-Sha256([string]$Path){(Get-FileHash -LiteralPath $Path -Algorithm SHA256).Hash.ToUpperInvariant()}
function Normalize-Lf([string]$Text){$Text.Replace("`r`n","`n").Replace("`r","`n")}
function Write-Utf8Bom([string]$Path,[string]$Text){[IO.File]::WriteAllText($Path,$Text,(New-Object System.Text.UTF8Encoding($true)))}
function Require-Token([string]$Text,[string]$Token,[string]$Name){if(-not $Text.Contains($Token)){throw "ANDROID_EVIDENCE_ROOTFIX_V24=FAIL verify=$Name token=$Token"}}
function Replace-LiteralOne([string]$Text,[string]$Needle,[string]$Replacement,[string]$Name){
  $first=$Text.IndexOf($Needle,[StringComparison]::Ordinal)
  if($first -lt 0){throw "ANDROID_EVIDENCE_ROOTFIX_V24=FAIL patch=$Name literal_missing"}
  $second=$Text.IndexOf($Needle,$first+$Needle.Length,[StringComparison]::Ordinal)
  if($second -ge 0){throw "ANDROID_EVIDENCE_ROOTFIX_V24=FAIL patch=$Name literal_ambiguous"}
  Write-Host "EVIDENCE_ROOTFIX_V24_HOOK=PASS name=$Name matches=1"
...
function Restore-OwnedFiles {
  ...
  if([string]$manifest.source_sha -ne $ExpectedSha){throw "ANDROID_EVIDENCE_ROOTFIX_V24=FAIL restore_manifest_sha expected=$ExpectedSha actual=$($manifest.source_sha)"}
  foreach($entry in @($manifest.files)){
    $target=Join-Path $RepoRoot ([string]$entry.path)
    $backup=Join-Path $backupRoot ([string]$entry.backup)
    if(-not(Test-Path -LiteralPath $backup -PathType Leaf)){throw "ANDROID_EVIDENCE_ROOTFIX_V24=FAIL restore_backup_missing path=$($entry.path)"}
    Copy-Item -LiteralPath $backup -Destination $target -Force
    $actualHash=Get-Sha256 $target
    $expectedHash=([string]$entry.sha256).ToUpperInvariant()
    if($actualHash -ne $expectedHash){throw "ANDROID_EVIDENCE_ROOTFIX_V24=FAIL restore_hash path=$($entry.path) expected=$expectedHash actual=$actualHash"}
  }
...
if($Mode -eq 'Restore'){
  ...
  if($LASTEXITCODE -ne 0){throw "ANDROID_EVIDENCE_ROOTFIX_V24=FAIL predecessor_restore_exit=$LASTEXITCODE"}
  Write-Host "ANDROID_EVIDENCE_ROOTFIX_V24=PASS mode=restore predecessor=v22 sha=$ExpectedSha"
  return
}

& $v22 -RepoRoot $RepoRoot -ExpectedSha $ExpectedSha -GitPath $GitPath -Mode Apply
if($LASTEXITCODE -ne 0){throw "ANDROID_EVIDENCE_ROOTFIX_V24=FAIL predecessor_apply_exit=$LASTEXITCODE"}

try {
  foreach($path in @($pausePath,$patcherPath)){
    if(-not(Test-Path -LiteralPath $path -PathType Leaf)){throw "ANDROID_EVIDENCE_ROOTFIX_V24=FAIL required_file_missing path=$path"}
  }
  ...
  [ordered]@{schema='armyattack-android-evidence-rootfix-overlay/v24';source_sha=$ExpectedSha;predecessor='v22';files=$files}|ConvertTo-Json -Depth 5|Set-Content -LiteralPath $manifestPath -Encoding UTF8

  # Physical evidence from c1fecfe7 proves the document picker opens, but after
  # selecting a file no SELECT/COMMIT/REJECTED event reaches the game. V22 held
  # the picker File only in a local variable and used a weak SELECT listener.
  # Keep both the File and the callback strongly reachable until SELECT/CANCEL.
...
'@.TrimEnd()
  ...
  if($pause.Contains('var mobileFile:File = new File();')){throw 'ANDROID_EVIDENCE_ROOTFIX_V24=FAIL local_picker_reference_survived'}
  if($pause.Contains('onExternalSaveSelected, false, 0, true')){throw 'ANDROID_EVIDENCE_ROOTFIX_V24=FAIL weak_select_listener_survived'}
  Write-Utf8Bom $pausePath $pause

  # Prove in the final SWF that the new callback/cancel telemetry strings were
  # compiled. String constants are stable across FFDec re-export.
  ...
  Write-Host 'ANDROID_EVIDENCE_ROOTFIX_V24_PICKER_LIFETIME=PASS root_cause=local_File_plus_weak_listener strong_member=true select_listener_weak=false cancel_cleanup=true selected_telemetry=true final_swf_markers=true'
  Write-Host "ANDROID_EVIDENCE_ROOTFIX_V24=PASS mode=apply predecessor=v22 sha=$ExpectedSha"
}
catch {
  $failure=$_
  try{Restore-OwnedFiles}catch{Write-Host "ANDROID_EVIDENCE_ROOTFIX_V24_RESTORE_OWNED_AFTER_FAILURE=FAIL message=$($_.Exception.Message)"}
  try{& $v22 -RepoRoot $RepoRoot -ExpectedSha $ExpectedSha -GitPath $GitPath -Mode Restore}catch{Write-Host "ANDROID_EVIDENCE_ROOTFIX_V24_PREDECESSOR_RESTORE_AFTER_FAILURE=FAIL message=$($_.Exception.Message)"}
  throw $failure
}
