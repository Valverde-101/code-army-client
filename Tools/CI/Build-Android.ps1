...
if([string]$patchManifest.patch_version -ne 'mobile-engine-v3'){throw "SWF_PERFORMANCE_PATCH=FAIL manifest_version=$($patchManifest.patch_version)"}
...
Write-Host "SWF_PERFORMANCE_PATCH=PASS version=mobile-engine-v3 patched_sha256=$swfSha size=$swfSize"
...
$prov=[ordered]@{
  ...
  performance_patch_version='mobile-engine-v3'
  performance_patch_manifest=$patchManifestPath
  performance_patch_classes=@('game.battlefield.TileMapGraphic','game.isometric.IsometricScene','Config','game.missions.MissionManager','game.gui.button.ArmyButton','game.characters.AnimationController','game.environment.EnvEffectManager')
...
Write-Host "BASE_ONLY_BUILD=PASS version=23.2 root_swf=$appContentSwf mods=false selector=false diagnostics_ane=true swf_source_original=true swf_performance_patched=true performance_patch=mobile-engine-v3 native_perf_overlay=true render_mode=$renderMode"
