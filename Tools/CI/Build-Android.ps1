...
$runtimeRegression=Join-Path $RepoRoot 'Tools\CI\Test-AndroidRuntimePatch.ps1'
if(-not (Test-Path -LiteralPath $runtimeRegression)){throw "REGRESSION=FAIL runtime_patch_test_missing=$runtimeRegression"}
& $runtimeRegression -RepoRoot $RepoRoot
...
if([string]$patchManifest.patch_version -ne 'mobile-engine-v3.7-save-onboarding'){throw "SWF_PERFORMANCE_PATCH=FAIL manifest_version=$($patchManifest.patch_version)"}
...
Write-Host "SWF_PERFORMANCE_PATCH=PASS version=mobile-engine-v3.7-save-onboarding patched_sha256=$swfSha size=$swfSize"
...
$runtimeConfigPath=Join-Path $stage 'config\army_config_base.json'
$runtimeOpponentsPath=Join-Path $stage 'config\army_config_pvp_opponents.json'
if(-not (Test-Path -LiteralPath $runtimeConfigPath)){throw "ANDROID_RUNTIME_CONFIG=FAIL missing=$runtimeConfigPath"}
if(-not (Test-Path -LiteralPath $runtimeOpponentsPath)){throw "ANDROID_RUNTIME_CONFIG=FAIL missing=$runtimeOpponentsPath"}
$runtimeConfig=Get-Content -LiteralPath $runtimeConfigPath -Raw|ConvertFrom-Json
$runtimeOpponents=Get-Content -LiteralPath $runtimeOpponentsPath -Raw|ConvertFrom-Json
if(-not $runtimeConfig.MapSetup.Desert -or [string]$runtimeConfig.MapSetup.Desert.TilemapFileName -ne 'tile_map_desert.csv'){throw 'ANDROID_RUNTIME_CONFIG=FAIL desert_map'}
if(@($runtimeConfig.PVPAreaSetup.PSObject.Properties).Count -lt 100){throw 'ANDROID_RUNTIME_CONFIG=FAIL pvp_area_setup'}
if(@($runtimeConfig.BadassLevels.PSObject.Properties).Count -lt 100){throw 'ANDROID_RUNTIME_CONFIG=FAIL badass_levels'}
...
$prov=[ordered]@{
  ...
  performance_patch_version='mobile-engine-v3.7-save-onboarding'
  performance_patch_manifest=$patchManifestPath
  performance_patch_classes=@('game.battlefield.TileMapGraphic','game.isometric.IsometricScene','game.utils.OfflineSave','game.states.GameState','game.gui.GiveFilePermissionDialog','game.net.PvPMatch','game.gui.popups.WorldMapWindow','game.gui.pvp.PvPMatchUpDialog','game.gui.pvp.PvPCombatSetupDialog')
...
Write-Host "BASE_ONLY_BUILD=PASS version=23.2 root_swf=$appContentSwf mods=false selector=false diagnostics_ane=true swf_source_original=true swf_performance_patched=true performance_patch=mobile-engine-v3.7-save-onboarding native_perf_overlay=true render_mode=$renderMode"
