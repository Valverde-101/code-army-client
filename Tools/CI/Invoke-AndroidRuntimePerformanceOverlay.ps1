...
$targets=@('src\game\battlefield\TileMapGraphic.as','src\game\isometric\IsometricScene.as')
...
function Replace-Exact([string]$Text,[string]$Old,[string]$New,[string]$Name){
  ...
  if($i -lt 0){throw "ANDROID_PERF_OVERLAY=FAIL patch=$Name reason=pattern_missing"}
  if($Text.IndexOf($Old,$i+$Old.Length,[System.StringComparison]::Ordinal) -ge 0){throw "ANDROID_PERF_OVERLAY=FAIL patch=$Name reason=pattern_ambiguous"}
  $Text.Substring(0,$i)+$New+$Text.Substring($i+$Old.Length)
}
function Replace-Range([string]$Text,[string]$Start,[string]$End,[string]$New,[string]$Name){
  $s=$Text.IndexOf($Start,[System.StringComparison]::Ordinal)
  if($s -lt 0){throw "ANDROID_PERF_OVERLAY=FAIL patch=$Name reason=start_missing"}
  if($Text.IndexOf($Start,$s+$Start.Length,[System.StringComparison]::Ordinal) -ge 0){throw "ANDROID_PERF_OVERLAY=FAIL patch=$Name reason=start_ambiguous"}
  $e=$Text.IndexOf($End,$s+$Start.Length,[System.StringComparison]::Ordinal)
  if($e -lt 0){throw "ANDROID_PERF_OVERLAY=FAIL patch=$Name reason=end_missing"}
  $after=$e+$End.Length
  $Text.Substring(0,$s)+$New+$Text.Substring($after)
}
...
if($Mode -eq 'Restore'){
  if(-not(Test-Path -LiteralPath $manifestPath -PathType Leaf)){Write-Host "ANDROID_PERF_OVERLAY=PASS mode=restore status=no_overlay sha=$ExpectedSha";return}
  $manifest=Get-Content -LiteralPath $manifestPath -Raw|ConvertFrom-Json
  foreach($entry in @($manifest.files)){
    $src=Join-Path $backupRoot ([string]$entry.backup);$dst=Join-Path $RepoRoot ([string]$entry.path)
...
$manifest=[ordered]@{schema='armyattack-runtime-performance-overlay/v2';source_sha=$ExpectedSha;files=@()}
foreach($rel in $targets){
  $src=Join-Path $RepoRoot $rel
  if(-not(Test-Path -LiteralPath $src -PathType Leaf)){throw "ANDROID_PERF_OVERLAY=FAIL source_missing=$rel"}
  $backup=($rel -replace '[\\/]','__')+'.original';Copy-Item -LiteralPath $src -Destination (Join-Path $backupRoot $backup) -Force
  $manifest.files+=@([ordered]@{path=$rel;backup=$backup;sha256=(Get-Sha256 $src)})
}
$manifest|ConvertTo-Json -Depth 6|Set-Content -LiteralPath $manifestPath -Encoding UTF8
try{
  $tilePath=Join-Path $RepoRoot 'src\game\battlefield\TileMapGraphic.as';$tile=Normalize-Lf ([IO.File]::ReadAllText($tilePath))
  $tile=Replace-Range $tile '      private static const CAMERA_CACHE_MARGIN:int = 256;' '      private var mCameraCacheAnchorY:Number = Number.NaN;' @'
...
'@ 'tile_cache_fields'
  $tile=Replace-Range $tile '         var cacheScale:Number = Math.max(0.01,this.mScene.mContainer.scaleX);' '         _loc4_ = new Rectangle(cacheLeft,cacheTop,cacheRight - cacheLeft,cacheBottom - cacheTop);' @'
...
'@ 'tile_last_draw_metrics'
  $tile=Replace-Range $tile '         var limit:Number = CAMERA_CACHE_MARGIN * CAMERA_CACHE_REBUILD_LIMIT;' '         if(Math.abs(dx) >= limit || Math.abs(dy) >= limit)' @'
...
'@ 'tile_effective_rebuild_threshold'
  $tile=Replace-Exact $tile '      private function disposeCachedTileBitmaps() : void' @'
...
'@ 'tile_metric_accessors'
  ...
  if($sceneVerify.Contains('this.reportSceneSubsystem("tilemap_redraw",getTimer() - perfStart,this.mGame.mMapData.mGrid ? this.mGame.mMapData.mGrid.length : 0);')){throw 'ANDROID_PERF_OVERLAY=FAIL stale_full_grid_telemetry'}
