...
try {
  ...
  Require $gameState 'ENEMY_DEAD_AUX_ACTION_SKIPPED' 'dead_enemy_auxiliary_action_lane_released_final'
  ...
  Require $gameState 'MOVE_COMMAND_DEFERRED_ENEMY_MOVE' 'snow_valid_player_walk_deferred_while_enemy_moves_final'
  Require $gameState 'MOVE_COMMAND_ALREADY_PENDING' 'snow_duplicate_deferred_walk_guard_final'
  Require $gameState 'SPAWN_WAVE_CHECK' 'spawn_wave_bounded_population_telemetry_final'
  Require $gameState 'SPAWN_WAVE_INTERVAL_INVALID' 'spawn_invalid_interval_guard_final'
  Require $gameState 'SPAWN_WAVE_BACKLOG_DROPPED' 'spawn_catchup_bounded_final'
  Require $gameState 'seenSpawnCells' 'spawn_dedup_final'
  Require $gameState '_loc5_.splice(_loc9_,1)' 'spawn_unique_cell_consumption_final'
  Require $gameState 'SPAWN_WAVE_PARACHUTE' 'periodic_airdrop_final'
  Require $enemy 'AIRDROP_LANDING_TIMEOUT' 'airdrop_landing_watchdog_final'
  Require $enemy 'this.mAirDropStartedAt = getTimer();' 'airdrop_start_clock_final'
  Require $attack 'var _loc16_: GridCell = targetCellForStack;' 'lethal_hit_position_snapshot_final'
  Write-Host 'REGRESSION_CHECK=PASS name=campaign_wave_source_contract interval_guard=true catchup_bounded=true cell_unique=true air_drop=true landing_watchdog=true lethal_hit_position_snapshot=true'
  Require $gameState 'TURRET_MANUAL_GROUP_VOLLEY' 'manual_turret_same_turn_group_support_final'
  Require $gameState 'volleySupportShot.mSameTurnTurretSupport = true;' 'manual_turret_support_tag_final'
  Require $gameState 'TURRET_GROUP_VOLLEY_BEFORE_ENEMY_RESPONSE' 'turret_volley_processed_before_enemy_round_final'
  Require $gameState 'MOVE_COMMAND_DEFERRED_BUSY_ACTION' 'campaign_busy_player_movement_preserved_final'
  Require $attack 'public var mSameTurnTurretSupport:Boolean = false;' 'turret_support_turn_tag_final'
  Write-Host 'REGRESSION_CHECK=PASS name=turret_support_turn_barrier_support_precedes_enemy_round=true valid_busy_walk_deferred=true'
  Require $attack 'CAMPAIGN_STACKED_DEFENCE_HIT' 'enemy_standing_on_own_defence_both_take_damage_final'
  Require $scene 'OWN_DEFENCE_UNIT_DRAW_PRIORITY' 'enemy_standing_on_own_defence_renders_in_front_final'
  Require $gameState 'MAP_SWITCH_COMBAT_RESET' 'campaign_map_switch_discards_old_turns_final'
  Require $gameState 'resetOfflineCampaignTurnLaneForMapSwitch(_loc2_,_loc1_)' 'campaign_map_switch_turn_reset_invoked_final'
  Require $gameState 'this.clearConcurrentEnemyActions();' 'campaign_map_switch_discards_auxiliary_actions_final'
  Require $gameState 'this.mOfflineEnemyResponseActive = false;' 'campaign_map_switch_old_response_reset_final'
