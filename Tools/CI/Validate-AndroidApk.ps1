...
if([string]$prov.performance_patch_version -ne 'mobile-engine-v3.6-pvp-map-memory'){$failures.Add("performance_patch_version=$($prov.performance_patch_version)")}
$patchClasses=@($prov.performance_patch_classes)
$allowedPatchClasses=@('game.battlefield.TileMapGraphic','game.isometric.IsometricScene','game.utils.OfflineSave','game.states.GameState','game.net.PvPMatch','game.gui.popups.WorldMapWindow','game.gui.pvp.PvPMatchUpDialog','game.gui.pvp.PvPCombatSetupDialog')
...
Write-Host "SWF_PERFORMANCE_PATCH_VALIDATE=PASS source_sha256=$canonicalSwfSha patched_sha256=$seedHash version=mobile-engine-v3.6-pvp-map-memory"
