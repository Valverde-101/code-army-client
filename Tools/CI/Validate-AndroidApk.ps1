...
if([string]$prov.performance_patch_version -ne 'mobile-engine-v3.8-map-pvp-render'){$failures.Add("performance_patch_version=$($prov.performance_patch_version)")}
$patchClasses=@($prov.performance_patch_classes)
$allowedPatchClasses=@('game.battlefield.TileMapGraphic','game.isometric.IsometricScene','game.isometric.characters.IsometricCharacter','Utils','game.utils.OfflineSave','game.states.GameState','game.gui.GiveFilePermissionDialog','game.net.PvPMatch','game.gui.popups.WorldMapWindow','game.gui.pvp.PvPMatchUpDialog','game.gui.pvp.PvPCombatSetupDialog','game.gui.pvp.PvPHUD')
...
Write-Host "SWF_PERFORMANCE_PATCH_VALIDATE=PASS source_sha256=$canonicalSwfSha patched_sha256=$seedHash version=mobile-engine-v3.8-map-pvp-render"
