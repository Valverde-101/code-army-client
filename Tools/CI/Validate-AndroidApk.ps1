...
if([string]$prov.performance_patch_version -ne 'mobile-engine-v3.1'){$failures.Add("performance_patch_version=$($prov.performance_patch_version)")}
$patchClasses=@($prov.performance_patch_classes)
foreach($requiredPatchClass in @('game.battlefield.TileMapGraphic','game.isometric.IsometricScene','Config','game.gui.button.ArmyButton','game.characters.AnimationController','game.environment.EnvEffectManager')){
...
Write-Host "SWF_PERFORMANCE_PATCH_VALIDATE=PASS source_sha256=$canonicalSwfSha patched_sha256=$seedHash version=mobile-engine-v3.1"
