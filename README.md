...
- Cada acción válida del jugador abre una respuesta enemiga de tres unidades principales. La orden de movimiento válida que llegue mientras se ejecuta otra acción de combate queda en cola una sola vez y se ejecuta tras terminar la respuesta enemiga; no se pierde por la ocupación temporal de la cola global.
- Una torreta seleccionada por el jugador dispara a un enemigo a su alcance y consume un solo turno del jugador. Las demás torretas aliadas vivas que también tengan ese objetivo a su alcance se incorporan como ataques de apoyo **sin consumir turnos adicionales**. Todos los disparos de esa misma salva se procesan **antes** de activar las tres unidades principales de respuesta enemiga; si el objetivo ya murió, los disparos posteriores se descartan sin perder el turno. El disparo automático por entrada en alcance se mantiene independiente.
...
- El temporizador de reposición no admite intervalos nulos o inválidos ni acumula oleadas tras una pausa. En campaña offline nace como máximo una oleada por actualización, de hasta cuatro enemigos, respetando el límite de 64 enemigos vivos por mapa. Los enemigos de saves antiguos no se eliminan artificialmente.
- Cada enemigo que nace en una oleada periódica debe descender en paracaídas sobre una casilla libre y única. La casilla se vuelve a comprobar justo antes de crear la unidad; después de aterrizar entran en acción las torretas a rango. Si la animación no termina en 7 segundos, el aterrizaje finaliza sin bloquear los turnos. La funcionalidad real y la animación deben probarse en APK físico.
- Antes de resolver un impacto se conservan las coordenadas del objetivo: su muerte no puede dejar el ataque ni los ataques de apoyo sin casilla válida.

- Una unidad enemiga que termina visualmente su movimiento sobre una casilla del jugador captura su propiedad **en ese mismo fotograma**, sin esperar al cierre del turno ni al comienzo de otra ronda. El borde y el color corresponden al nuevo propietario antes de resolver el siguiente ataque.
- En campaña, un movimiento enemigo no puede tener origen y destino separados por más casillas en ningún eje que el alcance MovementRange de esa unidad. Registrar CAMPAIGN_ENEMY_MOVE_RANGE con origen, destino y alcance; rechazar y registrar cualquier orden que lo incumpla sin bloquear los turnos.
- A todo nivel de zoom, los bitmaps de terreno deben reservar el tamaño transformado del recurso, sin recortar sus bordes ni producir líneas entre casillas. Cambiar el zoom no altera la propiedad territorial ni las acciones en curso.
- Al destruirse una mina propia debe sonar la **misma explosión que una mina enemiga**, una única vez por transición de vida positiva a cero, independientemente del bando que provocó el daño.
